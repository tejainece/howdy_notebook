...
metadata = {
    ...
    f"{name}.resnet_eps": str(downEncoder.resnets[0].norm1.eps),
    f"{name}.resnet_act_fn": str(downEncoder.resnets[0].nonlinearity.__class__.__name__),
    f"{name}.resnet_groups": str(downEncoder.resnets[0].norm1.num_groups),
    f"{name}.output_scale_factor": str(downEncoder.resnets[0].output_scale_factor),
    f"{name}.add_downsample": str(len(downEncoder.downsamplers) > 0),
    f"{name}.downsample_padding": str(downEncoder.downsamplers[0].padding if len(downEncoder.downsamplers) > 0 else 0),
}
